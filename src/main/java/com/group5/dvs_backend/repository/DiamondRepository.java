...
public interface DiamondRepository extends JpaRepository<Diamond, Integer> {
    @Query("SELECT d FROM Diamond d WHERE d.origin = :origin AND d.caratWeight = :carat AND d.shape = :shape AND d.color = :color AND d.clarity = :clarity")
    List<Diamond> findForFalseAdvanced(String origin, double carat, String shape, String color, String clarity);

    @Query("SELECT d FROM Diamond d WHERE d.origin = :origin AND d.caratWeight = :carat AND d.shape = :shape AND d.color = :color AND d.clarity = :clarity AND d.cut = :cut AND d.symmetry = :symmetry AND d.polish = :polish AND d.fluorescence = :fluorescence")
